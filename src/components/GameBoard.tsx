...
export function GameBoard() {
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden bg-play-area">
      <CommitmentEffect />
      {toastMessage && (
        <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-50 rounded-xl bg-felt border-2 border-cedar/50 px-4 py-2 shadow-xl text-cedar-dark text-base">
          {toastMessage}
        </div>
      )}
      {botThinking && (
      ...
      <header className="shrink-0 flex justify-between items-center px-4 py-3 sm:py-2 gap-2 flex-wrap bg-black/20 backdrop-blur-sm border-b border-cedar/30">
        <div className="flex items-center gap-3">
          <div className="border-b-2 border-lebanon-red pb-0.5">
            <h1 className="text-xl font-bold text-white font-heading drop-shadow-md">Mix Bitch</h1>
          </div>
          {matchState && matchState.mode !== 'single' && (
            <span className="rounded-lg bg-white/10 border border-cedar/30 px-3 py-1 text-sm font-medium text-white/90">
              Match: You {matchState.humanScore} – {matchState.botScore} Bot
            </span>
          )}
        </div>
        <div className="flex items-center gap-3 text-white/95 flex-wrap">
          {state.phase === 'play' && state.players[state.currentPlayerIndex]?.type === 'bot' && (() => {
            ...
            return (
              <button
                ...
                aria-label={humanOut ? 'Skip to end' : 'Skip to my turn'}
              >
                {skippingToMyTurn
                  ? (humanOut ? 'Skipping to end…' : 'Skipping…')
                  : (humanOut ? 'Skip to end' : 'Skip to my turn')}
              </button>
            )
          })()}
          <button
      ...
      <div
        className={`flex-1 min-h-0 overflow-hidden relative w-full rounded-none table-glow p-2 sm:p-0 ${layout.gapClass}`}
      >
        {/* Deck and pile counts — visible on all viewports */}
        <div
          className="absolute top-3 right-3 z-20 flex flex-row gap-2"
          aria-label="Deck and pile counts"
        >
        ...
        {state.phase === 'play' && !isMobile && <GameLog />}
        ...
        {/* All viewports: horizontal layout — opponents row, center, human bottom; no scroll */}
        <div className="absolute inset-0 flex flex-col min-h-0 overflow-hidden">
            ...
            {/* Center: pending hidden card (known to all), then PlayArea; on mobile also show embedded GameLog */}
            <div className="flex-1 min-h-0 flex flex-col sm:flex-row items-center justify-center gap-2 p-2">
              {state.phase === 'play' &&
                ...
                (() => {
                  ...
                  const card =
                    human?.hiddenCards[pendingHiddenReveal.cardIndex] ?? null
                  return card ? (
              ...
              {state.phase === 'play' && isMobile && (
                <div className="shrink-0 self-stretch min-h-0 flex items-center">
                  <GameLog embedded />
                </div>
              )}
            </div>
            {/* Human zone: bottom row */}
