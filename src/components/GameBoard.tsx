...
export function GameBoard() {
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden bg-play-area">
      ...
      <header className="shrink-0 flex justify-between items-center px-4 py-3 sm:py-2 gap-2 bg-black/20 backdrop-blur-sm border-b border-cedar/30 sm:flex-wrap">
      ...
      >
        ...
        {isMobile ? (
          <>
            {/* Mobile: scrollable middle + fixed human zone */}
            <div className="flex-1 min-h-0 overflow-y-auto overflow-x-hidden flex flex-col">
              <div className="sticky top-0 z-20 flex justify-end gap-2 p-2 bg-play-area/95 backdrop-blur-sm">
                <span className="rounded-lg bg-black/50 px-3 py-1.5 text-xs font-medium text-white shadow-lg" aria-label="Deck count">
                  Deck: {state.drawPile.length}
                </span>
                <span className="rounded-lg bg-black/50 px-3 py-1.5 text-xs font-medium text-white shadow-lg" aria-label="Pile count">
                  Pile: {state.commitmentPile.length}
                </span>
              </div>
              {lastBotCatchphrase && (
                <div className="w-full flex justify-center shrink-0 px-2">
                  <BotBubble
              ...
              <div className={`grid gap-2 px-2 py-1 justify-center ${n <= 2 ? 'grid-cols-2' : n <= 4 ? 'grid-cols-2' : 'grid-cols-2'}`}>
                {state.players
                  .filter((p) => p.id !== HUMAN_PLAYER_ID)
                  .map((p) => (
                    <div
                      key={p.id}
                      className={`rounded-xl bg-felt border-2 shadow-lg overflow-hidden min-w-0 ${
              ...
              <div className="flex flex-col items-center justify-center gap-2 p-2 shrink-0">
                {state.phase === 'play' &&
              ...
              {tutorialOn && (
                <div className="px-3 pb-2 max-w-md mx-auto w-full">
                  <TutorialPanel
