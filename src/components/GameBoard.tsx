...
export function GameBoard() {
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden bg-play-area">
      ...
      <header className="relative z-30 shrink-0 flex flex-col sm:flex-row sm:justify-between sm:items-center px-3 sm:px-4 py-2 sm:py-3 gap-1.5 sm:gap-2 bg-black/20 backdrop-blur-sm border-b border-cedar/30">
        <div className="flex items-center justify-between sm:justify-start gap-2 min-w-0 shrink-0">
        ...
        {/* Mobile: thin icon nav bar (no dropdown); full-width row below title */}
        <nav className="flex sm:hidden items-center gap-1 py-1.5 w-full overflow-x-auto min-h-0 shrink-0" aria-label="Game options">
          {state.phase === 'play' && state.players[state.currentPlayerIndex]?.type === 'bot' && state.players.find((p) => p.id === HUMAN_PLAYER_ID)?.commitmentLevel !== 'winner' && (
            <button
              type="button"
              onClick={skipToMyTurn}
              disabled={skippingToMyTurn}
              className="shrink-0 min-h-[36px] px-2 rounded text-white/95 hover:bg-white/10 focus:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-offset-1 focus-visible:ring-offset-cedar-dark disabled:opacity-60 text-xs font-medium"
              aria-label="Skip to my turn"
            >
              Skip
            </button>
          ...
          <button
            type="button"
            onClick={() => setShowHowToPlay(true)}
            className="shrink-0 min-h-[36px] min-w-[36px] flex items-center justify-center rounded text-white/95 hover:bg-white/10 focus:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-offset-1"
            aria-label="How to play"
            title="How to play"
          >
          ...
          <button
            type="button"
            onClick={() => { if (window.confirm('End this game? This round won\'t be saved.')) backToSetup() }}
            className="shrink-0 min-h-[36px] min-w-[36px] flex items-center justify-center rounded text-white/95 hover:bg-white/10 focus:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-offset-1"
            aria-label="End game"
            title="End game"
          >
          ...
          <label className="shrink-0 min-h-[36px] min-w-[36px] flex items-center justify-center rounded text-white/95 hover:bg-white/10 cursor-pointer focus-within:ring-2 focus-within:ring-white focus-within:ring-offset-1" title="Sound">
            <input
              type="checkbox"
              checked={soundOn}
              ...
              aria-label="Toggle sound"
            />
            <span className="text-base leading-none pointer-events-none" aria-hidden>{soundOn ? '🔊' : '🔈'}</span>
          </label>
          <label className="shrink-0 min-h-[36px] flex items-center gap-1 px-2 rounded text-white/95 hover:bg-white/10 cursor-pointer focus-within:ring-2 focus-within:ring-white focus-within:ring-offset-1" title="Tutorial hints">
            <input
              type="checkbox"
              checked={tutorialOn}
              onChange={(e) => setTutorialOn(e.target.checked)}
            ...
            />
            <span className="text-base leading-none pointer-events-none" aria-hidden>💡</span>
            <span className="text-xs font-medium text-white/95 whitespace-nowrap">Tutorial</span>
          </label>
          <button
            type="button"
            onClick={() => { const next = !darkMode; setDarkMode(next); setDark(next) }}
            className="shrink-0 min-h-[36px] min-w-[36px] flex items-center justify-center rounded text-white/95 hover:bg-white/10 focus:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-offset-1"
            aria-label={darkMode ? 'Light mode' : 'Dark mode'}
            title={darkMode ? 'Light mode' : 'Dark mode'}
          >
            <span className="text-base leading-none" aria-hidden>{darkMode ? '\u2600' : '\uD83C\uDF19'}</span>
          </button>
          <span className="shrink-0 px-2 py-1 text-xs text-white/80 capitalize" aria-label={`Phase: ${state.phase}`}>
            {state.phase}
          </span>
        </nav>
      </header>
              ...
              <div className="grid grid-cols-2 gap-2 px-2 pt-2 pb-1 justify-center max-h-[28vh] min-h-0 overflow-y-auto overflow-x-hidden">
                {state.players
                  ...
                  .map((p) => (
                    <div
                      key={p.id}
                      className={`rounded-xl bg-felt border-2 shadow-lg overflow-auto min-w-0 min-h-[100px] ${
