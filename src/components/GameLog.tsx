...
interface GameLogProps {
  ...
  embedded?: boolean
  /** When true (e.g. embedded on mobile), start collapsed so only one line shows. */
  defaultCollapsed?: boolean
}

export function GameLog({ embedded = false, defaultCollapsed = false }: GameLogProps) {
  const playLog = useGameStore((s) => s.playLog)
  const scrollRef = useRef<HTMLDivElement>(null)
  const [collapsed, setCollapsed] = useState(defaultCollapsed)
  const entries = playLog.slice(-MAX_VISIBLE)
  const lastEntry = entries.length > 0 ? entries[entries.length - 1] : null
  const collapsedLabel = lastEntry
    ? `Last: ${lastEntry.playerName} ${lastEntry.action === 'play' ? 'played' : 'collected'} ${lastEntry.summary}`
    : 'Activity'
    ...
    >
      ...
      >
        <span className="text-xs font-medium text-white/90 truncate flex-1 min-w-0">
          {collapsed ? collapsedLabel : 'Activity'}
        </span>
        <span className="text-white/60 text-xs shrink-0" aria-hidden>
          {collapsed ? '▼' : '▲'}
        </span>
      </button>
      {!collapsed && (
