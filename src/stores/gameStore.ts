...
function formatPlaySummary(cards: Card[]): string {
  ...
  const suitSym = getSuitSymbol(cards[0])
  const label = formatCardValue(v)
  const sameValue = cards.length > 1 && cards.every((c) => c.value === v)
  if (sameValue) return `${cards.length}× ${label}${suitSym}`
  if (cards.length === 1 && cards[0].commitmentType === 'void') return `${label}${suitSym} — void`
  if (cards.length === 1 && cards[0].commitmentType === 'reset') return `${label}${suitSym} — reset`
  if (cards.length === 1 && cards[0].commitmentType === 'reversal') return `${label}${suitSym} — reversal`
  if (cards.length === 1 && cards[0].commitmentType === 'echo') return `${label}${suitSym} — echo`
  if (cards.length === 1) return `${label}${suitSym}`
  return cards.map((c) => formatCardValue(c.value) + getSuitSymbol(c)).join(', ')
}
